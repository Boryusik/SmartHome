...
void ThreadPoolExecutor::submit(SharedPtr<IRunnable>const& a_task)
{
    m_queue.enqueue(a_task);
}

void ThreadPoolExecutor::cleanerSubmit(SharedPtr<IRunnable>const&  a_task)
{
    if(m_cleaner)
    {
...
SharedPtr<IRunnable> ThreadPoolExecutor::getTask()
{
...
void imp_details::ThreadPoolRunnable::run()
{
    while(true)
    {
        SharedPtr<IRunnable> task = m_tPool.getTask();
